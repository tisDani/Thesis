...
class HanamikojiEngine:
    ...
    def trade2x2(self, a1, a2 , b1 , b2 ):
        ...
        if self.oPlayer.name == 'Monte Carlo Expand' or self.oPlayer.name == 'Monte Carlo 2 Expand':
            self.decision = 'trade'
        ...
        elif self.oPlayer.name == 'Random 2' or self.oPlayer.name == 'Random 1' or self.oPlayer.name == 'Monte Carlo Rollout' or self.oPlayer.name == 'Monte Carlo 2 Rollout':
            inpt = random.randint(1,2)
    ...
    def trade3x1(self, a, b, c):
        ...
        if self.oPlayer.name == 'Monte Carlo Expand' or self.oPlayer.name == 'Monte Carlo 2 Expand':
            ...
            childs.append(['Na', out.parent_action])
            return 
        elif self.oPlayer.name == 'Random 1' or self.oPlayer.name == 'Random 2' or self.oPlayer.name == 'Monte Carlo Rollout' or self.oPlayer.name == 'Monte Carlo 2 Rollout':
            inpt = random.randint(1,3)
            print('Chooses: {}'.format(inpt))
        else:
...
class MonteCarloTreeSearchNode():
    ...
    def expand(self):
        ...
        self.original_state.player2.name = 'Monte Carlo 2 Expand'
    ...
    def rollout(self):
        while self.state.winner == 0:
            ...
            self.state.player2.name = 'Monte Carlo 2 Rollout'
    ...
    def move(self,action):  #action: [[],[]]
        ...
        if (self.state.player1.name == 'Monte Carlo Rollout' or self.state.player2.name == 'Monte Carlo 2 Rollout')and self.state.decision == 'trade':
            possible_moves = self.get_legal_actions(self.state)
            action = self.rollout_policy(possible_moves)
            l = self.state.monte_carlo(action)
        if len(self.state.deck.cards) != 0:
...
def MC_vs_random(gam):
    for i in range(1,5):  #5
        ...
        if gam.decision == 'trade':
            if len(gam.trade) == 4:  #2x2 trade
                gam.trade2x2(gam.trade[0], gam.trade[1], gam.trade[2], gam.trade[3])
            elif len(gam.trade) == 3:
                gam.trade3x1(gam.trade[0], gam.trade[1], gam.trade[2])
            gam.show_table()
        gam.cPlayer.draw(gam.deck, 1)
        outed = main('move', gam)        
        childs.append([i+10, outed.parent_action])
        #print('out')
        gam = outed.original_state
        gam.show_table()        
        if gam.decision == 'trade':
            if len(gam.trade) == 4:  #2x2 trade
                gam.trade2x2(gam.trade[0], gam.trade[1], gam.trade[2], gam.trade[3])
...
actual_moves = f"t_actual_moves{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
sim_childs = f"t_sim_childs{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"

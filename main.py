                ...
                for m in state.cPlayer.moves:
                    if m[1] == 0:
                        if m[0] == 'Trade 2x2':
                            ...
                            for index, dpair in enumerate(double_pairs):
                                for indexx, pair in enumerate(dpair):
                                    dpair[indexx] = sorted(pair)
                                double_pairs[index] = sorted(dpair)

                            [res.append(x) for x in double_pairs if x not in res]
                            for d_pair in res :
...
def main(decision, stat):
        ...
        childs.append([[0], root.original_state.cPlayer.hand.deck_id()])
...
def MC_vs_random(gam):
    for i in range(1,5):
        ...
        childs.append([[0], gam.cPlayer.hand.deck_id()])
        gam.random_move()
        #print('round over')
        #gam.show_table()
    return gam
